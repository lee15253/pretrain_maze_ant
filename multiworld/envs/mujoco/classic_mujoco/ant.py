...
class AntEnv(MujocoEnv, Serializable, MultitaskEnv, metaclass=abc.ABCMeta):
    ...
    def get_image_plt(
        ...
        # FIXME: 지워야함
        ax.add_artist(plt.Circle(np.array([0,0]), 0.5*marker_factor, color='red'))
        ax.add_artist(plt.Circle(np.array([1,0]), 0.5*marker_factor, color='black'))
        ax.add_artist(plt.Circle(np.array([0,1]), 0.5*marker_factor, color='yellow'))
